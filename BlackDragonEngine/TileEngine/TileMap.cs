...
{
    /// <summary>
    ///     The heart of the TileEngine, manages a Map object. This class implements a Pseudo-Singleton pattern. The first
    ///     instance has to be created via the public constructor. However, all instances you need afterwards have to be
    ///     fetched via the static GetInstance() method.
    /// </summary>
...
// ReSharper restore StaticFieldInGenericType
            ...
            uint layers = 1)
        {
            ...
            var tileCount = _tilesPerRow * _tileSheet.Height / (TileHeight + _tileOffset);
            ...
            _whiteTexture = new Texture2D(VariableProvider.GraphicsDevice, 1, 1);
        ...
        {
            var coords = _coordList[cellX, cellY];
        ...
        {
            var square = GetMapSquareAtCell(cell).GetValueOrDefault();
        ...
        public void SetTileAtCell(int tileX, int tileY, uint layer, int tileIndex)
        {
            var square = GetMapSquareAtCell(tileX, tileY).GetValueOrDefault();
        ...
        public void SetSolidTileAtCoords(Coords coords, int tileIndex)
        {
            ...
            var square = Map[coords].GetValueOrDefault();
        ...
        public void Draw(uint layer)
        {
            ...
            foreach (var coords in Map.MapData.Keys)
            {
                ...
                if (EditorMode) DrawEditModeItems(coords.X, coords.Y);
            }
        ...
        public void Draw()
        {
            ...
            {
                if (coords.X < startX || coords.X > endX || coords.Y < startY || coords.Y > endY) continue;
                for (var z = 0; z < MapSquare.Layers; ++z)
                {
                    var mapSquare = Map[coords];
                    Rectangle? sourceRect;
        ...
        public void DrawRectangleIndicator(MouseState ms, Vector2 startCell)
        {
            ...
            var cellX = MathHelper.Clamp(GetCellByPixelX((int) mouseLoc.X), 0, MapWidth - 1);
            var cellY = MathHelper.Clamp(GetCellByPixelY((int) mouseLoc.Y), 0, MapHeight - 1);
